...
quickswap_abi_path = 'abi/QuickSwapPair_V2.json'
pairABI = json.load(open(quickswap_abi_path))
quickswap_pairs_path = './quickswap_pairs_V2.json'
quickswap_blacklist_path = './quickswap_blacklist_V2.json'
...
def removeBlackList(pairs):
    blacklist = json.load(open(quickswap_blacklist_path))
    r = []
...
def send_message(trades, num_trades: int = 1, test_mode: bool = False):
    ...
    for data in trades:
        ...
        logger.info(f"Trade condition: {round(input_amount * Decimal('0.001'), 4)} WMATIC({round(usdt_input_amount * Decimal('0.001'), 4)} USDT)")
...
def main():
    start = time.time()
    all_pairs = json.load(open(quickswap_pairs_path))
